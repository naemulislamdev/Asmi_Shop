    ...
    public function addcartPost(Request $request)
    {
        ...
        if (!is_null($prod->max_qty) && $prod->max_qty > 0) {
            ...
            if (Session::has('cart')) {
                ...
                foreach ($existingCart->items as $cartItem) {
                    if ($cartItem['item']->id == $prod->id) {
            ...
            if ($currentQtyInCart >= $prod->max_qty) {
                return response()->json([
                    'error'   => true,
                    'message' => 'সর্বোচ্চ ' . $prod->max_qty . 'টি add করা যাবে।'
                ]);
            }
        }
            ...
            // Max qty check
                ...
                if (!is_null($product->max_qty) && $product->max_qty > 0) {
                    if ($cart->items[$key]['qty'] >= $product->max_qty) {
                        return response()->json([
                            'error'   => true,
                            'message' => 'সর্বোচ্চ ' . $product->max_qty . 'টি add করা যাবে।'
                        ]);
                    }
                }
