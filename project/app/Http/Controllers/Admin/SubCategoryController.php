...
class SubCategoryController extends AdminBaseController
{
    ...
    public function datatables()
    {
        ...
        return Datatables::of($datas)
            ...
            ->addColumn('featured', function (Subcategory $data) {
                $class = $data->featured == 1 ? 'drop-success' : 'drop-danger';
                $s = $data->featured == 1 ? 'selected' : '';
                $ns = $data->featured == 0 ? 'selected' : '';
                return '<div class="action-list"><select class="process select droplinks ' . $class . '"><option data-val="1" value="' . route('admin-subcat-featured', ['id1' => $data->id, 'id2' => 1]) . '" ' . $s . '>' . __("Activated") . '</option><<option data-val="0" value="' . route('admin-subcat-featured', ['id1' => $data->id, 'id2' => 0]) . '" ' . $ns . '>' . __("Deactivated") . '</option>/select></div>';
            })
