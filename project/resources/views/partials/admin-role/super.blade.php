...
<li>
    ...
    <li>
        <a href="{{ $item->branch_id 
            ? route('branch-orders.single', $item->branch_id) 
            : route('branch-orders.not-assigned') }}">
            {{ $item->branch_id ? ($item->branch->name ?? 'Unknown') : __('Not Assigned Branch') }}
            <span class="text-center align-self-center badge {{ $badges[$index % count($badges)] }} float-right">
                {{ $item->total ?? 0 }}
            </span>
        </a>
    </li>
