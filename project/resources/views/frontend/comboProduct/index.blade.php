...
@section('title', $comboProduct->title)
@section('css')
    <style>
        .gs-blog-wrapper {
            padding: 0 !important;
        ...
        <section class="category_banner" style="background: #EDEDED;">
            @if ($comboProduct->image)
                <div class="container productBannerContainer px-0 px-lg-2">
                    <div class="gs-breadcrumb-section  bg-class rounded-b"
                        data-background="{{ asset('assets/images/combo-offers/' . $comboProduct->image) }}">
                    </div>
                </div>
            @endif
    ...
    <div class="gs-blog-wrapper pt-3" style="background: #ededed">
        <div class="container">
            ...
            <nav class="custom-breadcrumb mb-2 mb-lg-0">
                ...
                @if (!empty($comboProduct->title))
                    <span class="separator"><i class="fa fa-chevron-right"></i></span>
                    <span class="active">{{ $comboProduct->title }}</span>
                @endif
            </nav>
            ...
            <div class="row flex-column-reverse flex-lg-row">
                ...
                <div class="col-lg-12 gs-main-blog-wrapper px-0">
                    ...
                    @endphp
                    @if (count(json_decode($comboProduct->product_ids)) == 0)
                    ...
                    @else
                        <div class="tab-content" id="myTabContent">
                            ...
                            <div class="tab-pane fade show active" id="layout-grid-pane" role="tabpanel" tabindex="0">
                                <div class="row gy-4 justify-content-center gx-4 mt-20">
                                    @if ($comboProduct->product_ids != null)
                                        @php
                                            $productIds = json_decode($comboProduct->product_ids, true) ?? [];
                                            $comboProducts = \App\Models\Product::whereIn('id', $productIds)->get();
                                        @endphp
                        ...
                        {{-- {{ count(json_decode($comboProduct))->links('includes.frontend.pagination') }} --}}
                    @endif
